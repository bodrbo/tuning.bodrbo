...
function admin_slugify(mixed $value): string
{
    $source = admin_text($value, 240, true);
    $transliteration = [
    ...
    $slug = trim(substr($slug, 0, 96), '-');

    return $slug !== '' ? $slug : 'novyi-keis';
}

function admin_unique_slug(string $base, array $projects): string
{
    $used = [];
    foreach ($projects as $project) {
        if (is_array($project) && is_string($project['slug'] ?? null)) {
            $used[$project['slug']] = true;
        }
    }

    $slug = $base;
    for ($suffix = 2; isset($used[$slug]); $suffix++) {
        $slug = rtrim(substr($base, 0, 91), '-') . '-' . $suffix;
    }

    return $slug;
}

function admin_unique_project_key(string $slug, array $projects): string
{
    $base = 'case-' . trim(substr($slug, 0, 70), '-');
    $key = $base;
    for ($suffix = 2; isset($projects[$key]); $suffix++) {
        $key = rtrim(substr($base, 0, 75), '-') . '-' . $suffix;
    }
